...
description: Delegate a coding or analysis task to a DeepSeek worker — via Codex through the opencodex proxy (primary) or the reasonix CLI (fallback) — then independently verify the result before reporting it done. Use when the user asks to hand off / delegate / offload work to DeepSeek, reasonix, codex, "flash", or "pro", or to have a second agent do work that Claude then double-checks.
---

# Delegate to a DeepSeek worker (Claude verifies)
...
Routing mechanics — adding providers, the dynamic port, `ocx restart`, catalog sync — live in the **codex-routed-provider** skill. Resuming an interrupted Codex session is the **codex-session-resume** skill. For a second *opinion* from a non-DeepSeek model, see **codex-kiro**.
...
- Never read or print secrets: `~/.reasonix/.env`, `~/.codex/auth.json`, `~/.opencodex/auth.json`, the `OPENCODE_GO_API_KEY` value, or any credential a worker's output might echo. Inspect such files with `jq -r 'keys[]'` when you need structure only.
